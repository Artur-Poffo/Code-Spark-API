...
import { makeQueryCoursesByTagsUseCase } from '@/infra/use-cases/factories/make-query-courses-by-tags-use-case'
...
const queryCoursesByTagQuerySchema = z.object({
  q: z.string()
})

export async function queryCoursesByTagController(request: FastifyRequest, reply: FastifyReply) {
  const { q } = queryCoursesByTagQuerySchema.parse(request.query)

  const queryCoursesByTagsUseCase = makeQueryCoursesByTagsUseCase()
  ...
  const result = await queryCoursesByTagsUseCase.exec({
    query: q
  })
